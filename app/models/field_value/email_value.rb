class FieldValue
  class EmailValue < ::FieldValue
    ...
    validates :value, format: { with: SINGLE_EMAIL_REGEXP },   allow_blank: true, unless: ->{ multiple_email_allowed? } # allowing nil is important in case there are no value provided (when optional)
    validates :value, format: { with: MULTIPLE_EMAIL_REGEXP }, allow_blank: true, if:     ->{ multiple_email_allowed? } # allowing nil is important in case there are no value provided (when optional)
